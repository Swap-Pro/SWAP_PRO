...
# ─────────────────────────── 1. Settings ───────────────────────────
TEMPLATE_PATH = os.getenv('TEMPLATE_PATH')      # Empty plate template
MODEL_PATH = os.getenv('MODEL_PATH')
OUT_PATH = os.getenv('OUT_PATH')                       # Final output
...
def generate_iran_plates():
    ...
    if random_char['ch'] in ['T', 'EIN']:
        background = os.getenv('TAXI_TEMPLATE')
    elif random_char['ch'] in ['P', 'TH']:
        background = os.getenv('POLICE_TEMPLATE')
        char_color = 'white'
    elif random_char['ch'] == 'A':
        background = os.getenv('GOV_TEMPLATE')
        char_color = 'white'
    elif random_char['ch'] == 'Z':
        background = os.getenv('BLUE_TEMPLATE')
        char_color = 'white'
    elif random_char['ch'] == 'SH':
        background = os.getenv('ARTESH_TEMPLATE')
    else:
        background = os.getenv('SAVARI_TEMPLATE')
...
def generate_egypt_plate():
    # تحميل الخط
    bold_font_path = os.getenv('FONT_PATH')
...
def generate_iraq_plate():
    # 1️⃣ تحميل الخلفية
    background_path = os.getenv('IRAQ_TEMPLATE')
    background = Image.open(background_path).convert("RGB")
    draw = ImageDraw.Draw(background)

    # 2️⃣ إعداد الخط
    font_path = os.getenv('FE_FONT_PATH')
...
def generate_qatar_plate():
    ...
    font_path = os.getenv('FIRA_FONT_PATH')
    # urllib.request.urlretrieve(font_url, font_path)

    background_path = os.getenv('QATAR_TEMPLATE')
    background = Image.open(background_path).convert("RGB")
    draw = ImageDraw.Draw(background)
...
def generate_germany_plate():
    def find_font_size_that_fits(max_height, max_width, space_after_b, space_after_letters, margin_right=30):
        ...
        while font_size > 0:
            fnt = ImageFont.truetype(os.getenv('FE_FONT_PATH'), font_size)
    ...
    def find_font_size(max_height):
        for size in range(10, 1000):
            fnt = ImageFont.truetype(os.getenv('FE_FONT_PATH'), size)
            bbox = fnt.getbbox("X")
            height = bbox[3] - bbox[1]
    ...
    ger_plate = os.getenv('GERMAN_PLATE')
...
def generate_germany_plate_bayern():
    ...
    saftey = os.getenv('BAYERN_SEAL')
    draw_image(img, (x, y + sticker1_y_offset), decal1_size, saftey, 0)
    bayern = os.getenv('BERLIN_SEAL')
...
def process_image(scene_bgr, country='IT'):
    ...
    VIGNETTE_STRENGTH = float(os.getenv('VIGNETTE_STRENGTH'))
    GAUSS_BLUR_K      = tuple(map(int, os.getenv('GAUSS_BLUR_K').split(',')))
    CLAHE_CLIP        = float(os.getenv('CLAHE_CLIP'))
    JPEG_QUALITY      = int(os.getenv('JPEG_QUALITY'))
    CENTER_MARGIN     = float(os.getenv('CENTER_MARGIN'))
    SIGMA_MIN, SIGMA_MAX = int(os.getenv('SIGMA_MIN')), int(os.getenv('SIGMA_MAX'))
